...
describe("Records Controller", () => {
    ...
    describe("getAllExperienceRecords", () => {
        test("It should return all experience records on success.", async () => {
            (RecordsService.prototype.getAllExperienceRecords as jest.Mock).mockResolvedValue(mockExperienceRecords);
        ...
        test("It should return a message if no experience records are found.", async () => {
            (RecordsService.prototype.getAllExperienceRecords as jest.Mock).mockResolvedValue(null);
        ...
        test("It should handle errors", async () => {
            (RecordsService.prototype.getAllExperienceRecords as jest.Mock).mockRejectedValue(
                new Error("Failed to retrieve experience records"),
            );

            const result = await RecordsController.getAllExperienceRecords();

            expect(result.status).toBe(500);
            expect(result.body).toBe("Failed to retrieve experience records");
        });
    });

    describe("getAllEducationalRecords", () => {
        test("It should return all educational records on success.", async () => {
            (RecordsService.prototype.getAllEducationalRecords as jest.Mock).mockResolvedValue(mockEducationRecords);
        ...
        test("It should return a message if no educational records are found.", async () => {
            (RecordsService.prototype.getAllEducationalRecords as jest.Mock).mockResolvedValue(null);
        ...
        test("It should handle errors", async () => {
            (RecordsService.prototype.getAllEducationalRecords as jest.Mock).mockRejectedValue(
                new Error("Failed to retrieve educational records"),
            );

            const result = await RecordsController.getAllEducationalRecords();

            expect(result.status).toBe(500);
            expect(result.body).toBe("Failed to retrieve educational records");
        });
    });
    ...
    describe("getExperienceRecordByID", () => {
        ...
        test("It should return the specified experience record with given ID on success.", async () => {
            ...
            (RecordsService.prototype.getExperienceRecordByID as jest.Mock).mockResolvedValue(mockExperienceRecord);
        ...
        test("It should return a message if no experience record is found with the given ID.", async () => {
            ...
            (RecordsService.prototype.getExperienceRecordByID as jest.Mock).mockResolvedValue(null);

            const result = await RecordsController.getExperienceRecordByID(testID);

            expect(result.status).toBe(200);
            expect(result.body).toBe("No Experience record fetched with ID: " + testID);
        });

        test("It should handle errors when no ID parameter is specified", async () => {
            (RecordsService.prototype.getExperienceRecordByID as jest.Mock).mockRejectedValue(
        ...
        test("It should handle errors", async () => {
            ...
            (RecordsService.prototype.getExperienceRecordByID as jest.Mock).mockRejectedValue(
                new Error("Failed to retrieve experience record with ID: " + testID),
            );

            const result = await RecordsController.getExperienceRecordByID(testID);

            expect(result.status).toBe(500);
            expect(result.body).toBe("Failed to retrieve experience record with ID: " + testID);
        });
    });
    ...
    describe("getEducationalRecordByID", () => {
        ...
        test("It should return the specified educational record with given ID on success.", async () => {
            ...
            (RecordsService.prototype.getEducationalRecordByID as jest.Mock).mockResolvedValue(mockEducationalRecord);
        ...
        test("It should return a message if no educational record is found with the given ID.", async () => {
            ...
            (RecordsService.prototype.getEducationalRecordByID as jest.Mock).mockResolvedValue(null);

            const result = await RecordsController.getEducationalRecordByID(testID);

            expect(result.status).toBe(200);
            expect(result.body).toBe("No Educational record fetched with ID: " + testID);
        });

        test("It should handle errors when no ID parameter is specified", async () => {
            (RecordsService.prototype.getEducationalRecordByID as jest.Mock).mockRejectedValue(
        ...
        test("It should handle errors", async () => {
            ...
            (RecordsService.prototype.getEducationalRecordByID as jest.Mock).mockRejectedValue(
                new Error("Failed to retrieve educational record with ID: " + testID),
            );

            const result = await RecordsController.getEducationalRecordByID(testID);

            expect(result.status).toBe(500);
            expect(result.body).toBe("Failed to retrieve educational record with ID: " + testID);
        });
    });
});